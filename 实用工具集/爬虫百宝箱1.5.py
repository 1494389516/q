...
class AsyncCrawler:
    ...
    def __init__(self, max_connections=100, timeout=30, retries=3):
        ...
        self.css_decryptor = CSSDecryptor()
    ...
    try:
        ...
        if img_url.startswith('data:image'):
            ...
            async with async_crawler.session.get(img_url, headers=headers_dict) as img_response:
                img_content = await img_response.read()
                captcha_result = async_crawler.captcha_solver.solve_image_captcha(img_content)
                
        if not captcha_result:
            print("验证码识别失败，无法自动绕过。")
            return
        ...
        if not captcha_field:
            for possible_name in ['captcha', 'validateCode', 'verifyCode', 'verification']:
                if soup.find('input', {'name': possible_name}):
                    captcha_field = possible_name
                    break
                    
        if not captcha_field:
            print("未找到验证码输入字段，使用默认字段名'captcha'")
            captcha_field = 'captcha'
            
        form_data[captcha_field] = captcha_result
