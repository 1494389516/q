...
async def try_api_endpoint(api_url, headers_dict, throttler, async_crawler, proxy_manager, config_data):
    """尝试请求API端点并解析评论数据"""
    try:
...
async def crawl_xiaohongshu_comments(
    ...
    config_data: Dict # Added config_data
):
    """专门针对小红书评论的爬取函数"""
    print("\n=== 小红书评论爬取 ===")
...
def load_config(config_path=CONFIG_FILE):
    ...
    default_config = {
        ...
        'splash_settings': { # New section for Splash
            'enabled': False,
            'server_address': "http://47.110.161.129:8050",
            'wait': 5.0
        }
    }
...
class CrawlerConfig:
    ...
    def __init__(self):
        ...
        # Splash settings
        self.splash_enabled: bool = False
        self.splash_server_address: str = "http://47.110.161.129:8050"
        self.splash_wait: float = 5.0
...
async def make_request_async(
    ...
    splash_wait: Optional[float] = None, # New parameter for Splash wait time
    config_data: Optional[Dict] = None # Added config_data
    ...
    if use_splash:
        ...
        if not config_data:
            logger.error("Splash无法使用，因为配置数据未提供给make_request_async")
            # Fallback to not using splash or handle error as appropriate
            # For now, we'll proceed without splash if config_data is missing
            pass # Or: return await async_crawler.fetch(url, ...original params...)
        else:
            splash_settings = config_data.get('splash_settings', {})
            splash_server = splash_settings.get('server_address', "http://47.110.161.129:8050")
            # Ensure splash_server ends with /render.html or similar if needed, or construct full URL
            if not splash_server.endswith('/execute') and not splash_server.endswith('/render.html'):
                splash_server = urljoin(splash_server, 'render.html') # Default to render.html
            
            splash_args = {
                'url': url,
                'wait': splash_wait if splash_wait is not None else splash_settings.get('wait', 5.0)
            }
            if proxy_url:
                splash_args['proxy'] = proxy_url # Pass proxy to Splash if used
                proxy_url = None # Splash handles the proxy, so don't pass it to aiohttp directly
            
            # Encode parameters for Splash URL
            splash_params_encoded = "&".join([f"{k}={v}" for k,v in splash_args.items()])
            request_url = f"{splash_server}?{splash_params_encoded}"
            request_params = None # Params are part of Splash URL now
            logger.info(f"使用Splash渲染: {request_url}")
...
async def parse_bilibili_video_async(
    bvid: str, 
    referer: str, 
    headers: Dict, 
    throttler: AsyncRequestThrottler, 
    ...
    try:
        ...
        response = await make_request_async(
            ...
            proxy_manager=proxy_manager,
            method='GET',
            config_data=config_data # Pass config_data
        )
...
async def crawl_images_revised_async(
    page_url, 
    html_content_for_images, 
    headers_dict, 
    throttler, 
    async_crawler, 
    recursive=True, 
    ...
    retry=None,
    config_data: Optional[Dict] = None, # Added config_data
    use_splash_for_recursive_fetches: bool = False, # Added for recursive calls
    splash_wait_for_recursive_fetches: Optional[float] = None # Added for recursive calls
):
    """
        ...
        if recursive and depth < max_depth:
            ...
            for link in filtered_links:
                if link not in link_extractor.visited_urls:
                    ...
                    # Pass config_data and splash parameters for recursive fetches
                    link_tasks.append(make_request_async(
                        link,
                        async_crawler=async_crawler,
                        headers=headers_dict,
                        proxy_manager=proxy_manager, 
                        throttler=throttler,
                        config_data=config_data,
                        use_splash=use_splash_for_recursive_fetches,
                        splash_wait=splash_wait_for_recursive_fetches
                    ))
...
async def crawl_bilibili_comments(target_url: str, headers_dict: Dict, throttler: 'AsyncRequestThrottler', async_crawler: 'AsyncCrawler', proxy_manager: Optional['AsyncProxyManager'], config_data: Dict):
    """专门针对B站评论的爬取函数"""
    print("\n=== B站评论爬取 ===")
...
async def crawl_comments_async(
    target_url: str, 
    headers_dict: Dict, 
    throttler: AsyncRequestThrottler, 
    async_crawler: AsyncCrawler, 
    proxy_manager: Optional[AsyncProxyManager],
    config_data: Dict, # Added config_data
    html_content_for_main_page: Optional[str] = None, # HTML from main, possibly Splash rendered
    use_splash_for_main_page_load_if_needed: bool = False, # If html_content is None, should we use Splash?
    splash_wait_for_main_page_load_if_needed: Optional[float] = None # Wait time for Splash if used
):
    """爬取评论的主函数"""
    ...
    try:
        if "bilibili.com" in target_url or "b23.tv" in target_url:
            ...
            await crawl_bilibili_comments(target_url, headers_dict, throttler, async_crawler, proxy_manager, config_data)
            return
        
        if "xiaohongshu.com" in target_url or "xhslink.com" in target_url:
            print("检测到小红书链接，使用专用爬取方法...")
            await crawl_xiaohongshu_comments(target_url, headers_dict, throttler, async_crawler, proxy_manager, config_data)
            return
        
        json_comments = await try_fetch_json_comments(target_url, headers_dict, throttler, async_crawler, proxy_manager, config_data)
        if json_comments:
            return
        
        html_content_to_parse = html_content_for_main_page

        if html_content_to_parse is None:
            try:
                await throttler.wait_async()
                response = await make_request_async(
                    target_url,
                    ...
                    method='GET',
                    config_data=config_data,
                    use_splash=use_splash_for_main_page_load_if_needed,
                    splash_wait=splash_wait_for_main_page_load_if_needed
                )
                if not response:
                ...
                if 'application/json' in content_type_header:
                    try:
                        json_data = await response.json()
                        if await parse_json_comments(json_data, target_url): # check if comments were found and saved
                            return
                ...
                try:
                    html_content_to_parse = await response.text(encoding=charset, errors='replace')
                except UnicodeDecodeError:
                    logger.warning(f"UnicodeDecodeError with detected charset {charset} for {target_url}, trying 'utf-8'")
                    html_content_to_parse = await response.text(encoding='utf-8', errors='replace')
            except aiohttp.ClientResponseError as e_http_fetch:
                print(f"获取页面时发生HTTP错误 {e_http_fetch.status}: {e_http_fetch.message} - URL: {target_url}")
            except Exception as e_fetch:
                print(f"获取页面时发生错误: {e_fetch} - URL: {target_url}")
                traceback.print_exc()
        
        if html_content_to_parse is None:
            print(f"由于无法获取或解析页面内容，评论爬取中止于 {target_url}")
            return

        api_endpoints = extract_api_endpoints(html_content_to_parse, target_url)
        if api_endpoints:
            ...
            if api_endpoints:
                ...
                for endpoint in api_endpoints[:2]:
                    if await try_api_endpoint(endpoint, headers_dict, throttler, async_crawler, proxy_manager, config_data): # If API endpoint successfully found and saved comments, we can potentially return
                        print(f"API端点 {endpoint} 可能已成功处理评论。")
                        # Depending on desired behavior, could return here if API comments are sufficient
        ...
        for element in soup.select('[class*="comment"], [class*="review"], .comment, .review-item'):
            try:
                ...
                if content and len(content) > 10: # Basic filter for meaningful comments
                    comment_data.append({
                        "user": user,
                        "time": time,
                        "content": content
                    })
            except Exception: # Ignore errors in parsing individual comment items
                continue
        ...
        if comment_data:
            ...
            return # Successfully processed and saved user-info comments
        ...
        for selector in comment_selectors:
            ...
            for element in elements:
                ...
                # Avoid double-counting if a selector matches a container and its child
                ...
                if not is_nested_comment_container:
                    ...
                    if comment_text and len(comment_text) > 10: # Basic filter
                        found_comments.append(comment_text)
                        
        if found_comments:
            # Use set to remove duplicates before displaying/saving
            unique_found_comments = list(set(found_comments))
            print(f"\n找到 {len(unique_found_comments)} 条可能的评论内容 (已去重):")
            for i, comment in enumerate(unique_found_comments[:10]):  # 只显示前10条
                print(f"--- 评论 {i+1} ---")
                print(comment)
                print("--------------------")
            
            # 保存简单评论数据
            simple_comment_data = [{"content": c} for c in unique_found_comments]
            ...
            for script in scripts:
                ...
                if script_text and ("comment" in script_text.lower() or "评论" in script_text):
                    ...
                    # 尝试从脚本中提取JSON数据
                    json_matches = re.findall(r'({[\\s\\S]*?})', script_text) # Non-greedy match for JSON
                    for json_str in json_matches[:3]:  # 只检查前3个匹配项
                        try:
                            # Basic validation: starts with { and ends with }
                            if json_str.startswith('{') and json_str.endswith('}'):
                                json_data = json.loads(json_str)
                                # Further check if it looks like comment data
                                if isinstance(json_data, dict) and any(k for k in json_data.keys() if "comment" in str(k).lower() or "reply" in str(k).lower()):
                                    print("从脚本中提取到可能的评论JSON数据:")
                                    print(json.dumps(json_data, ensure_ascii=False, indent=2)[:500] + "...")  # 只显示前500个字符
                                    # Potentially call parse_json_comments here if structure is known
                                    if await parse_json_comments(json_data, target_url + "#script_extracted"):
                                        print("已通过脚本提取的JSON数据保存评论。")
                                    break # Found potential JSON in a script
                        except json.JSONDecodeError:
                            # Not a valid JSON object, common for script content
                            continue 
                        except Exception: # Other errors during processing
                            continue
                    break # Processed first script with "comment" or "评论"
...
async def crawl_videos_async(
    ...
    # 异步下载视频
    async def download_video(video_url, index, current_config_data: Optional[Dict]): # Added current_config_data
        """异步下载单个视频"""
            ...
            for attempt in range(3):  # 最多重试3次
                try:
                    ...
                    response = await make_request_async(
                        ...
                        proxy_manager=proxy_manager,
                        method='GET',
                        config_data=current_config_data # Pass config_data
                    )
    ...
    for i, video_url in enumerate(all_urls):
        download_tasks.append(download_video(video_url, i, config_data)) # Pass config_data to helper
...
async def async_main():
    ...
    async with AsyncCrawler(max_connections=10, timeout=DEFAULT_TIMEOUT) as async_crawler_instance:
            ...
            # Ask user if they want to use Splash for the main request
            use_splash_for_main_request_str = input("是否为此主请求使用Splash渲染JavaScript? (yes/no, 默认no): ").strip().lower()
            use_splash_for_main_request = use_splash_for_main_request_str == 'yes'
            splash_wait_time = None
            if use_splash_for_main_request:
                try:
                    splash_wait_input = input(f"请输入Splash等待时间 (秒), 默认 {config_data.get('splash_settings', {}).get('wait', 5.0)}: ").strip()
                    if splash_wait_input:
                        splash_wait_time = float(splash_wait_input)
                except ValueError:
                    print(f"无效的等待时间，将使用默认值。")
            ...
            if choice == '4':
                try:
                    await crawl_douban_top250_movies_async(
                        ...
                        use_splash_for_main_request, # Pass Splash choice
                        splash_wait_time # Pass Splash wait time
                    )
                        ...
                        if choice == '3' and ('bilibili.com' in processed_target_url or 'b23.tv' in processed_target_url):
                            ...
                            try:
                                ...
                                response = await make_request_async(
                                    ...
                                    config_data=config_data, # Pass config_data
                                    use_splash=use_splash_for_main_request, # Use Splash if chosen
                                    splash_wait=splash_wait_time # Pass Splash wait time
                                )
                            ...
                            if html_content_main:
                                if choice == '1':
                                    ...
                                    await crawl_images_revised_async(
                                        ...
                                        proxy_manager=async_proxy_manager,
                                        config_data=config_data # Pass config_data
                                        # crawl_images_revised_async itself doesn't need use_splash for its direct operations
                                        # as it operates on already fetched HTML. Splash is applied when fetching html_content_main.
                                    )
                                elif choice == '2':
                                    print("\n--- 开始爬取评论 ---")
                                    # crawl_comments_async will use the html_content_main (fetched with/without Splash)
                                    # and then make its own API calls (which should not use Splash)
                                    await crawl_comments_async(processed_target_url, custom_headers, default_async_throttler, async_crawler_instance, async_proxy_manager, config_data, html_content_main, use_splash_for_main_request, splash_wait_time)
                                elif choice == '3':
                                    print("\n--- 开始爬取视频到本地 (通用方法) ---")
                                    await crawl_videos_async(
                                        processed_target_url,
                                        custom_headers,
                                        html_content_main, # Fetched with or without Splash
                                        default_async_throttler,
                                        async_crawler_instance,
                                        async_proxy_manager,
                                        config_data=config_data # Pass config_data
                            ...
                            print("\n--- 开始爬取文档内容 ---")
                            await crawl_document_async(processed_target_url, custom_headers, default_async_throttler, async_crawler_instance, async_proxy_manager, config_data, use_splash_for_main_request, splash_wait_time)
                    else:
                        print("无效的选项。程序将退出。")
                        tasks_increment_counter = False
